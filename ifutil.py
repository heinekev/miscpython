...
Created by Anderson, Kevin on 2012-07-12.
...
class SnmpManager:
	...
	def query_all_systems(self):
		...
		for system in self.systems.values():
			comm_data = cmdgen.CommunityData('ifutil.py', system['communityro'])
...
def main(conf_file=""):
	...
	for system in [s for s in config.sections() if s.startswith('system')]:
		snmp_manager.add_system(system, config.get(system, 'description'), config.get(system, 'address'), config.get(system, 'port'),config.get(system, 'communityro'))
	for check in [c for c in config.sections() if c.startswith('check')]:
		snmp_manager.add_check(check, config.get(check, 'oid'), config.get(check, 'description'), config.get(check, 'system'))
	snmp_manager.query_all_systems()

if __name__ == '__main__':
	main(conf_file='config.conf')